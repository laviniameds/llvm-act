...
//  static cl::opt<unsigned> MaxIntegerBW("float2int-max-integer-bw", cl::init(64), cl::Hidden, 
...
int MaxIntegerBW = 16;
 ...
 void Float2IntPass::walkForwards() {
   for (auto &It : reverse(SeenInsts)) {
    ...
    //      auto Zero = ConstantRange(APInt::zextOrSelf(Size));
 ...
 bool Float2IntPass::validateAndTransform() {
   ...
   for (auto It = ECs.begin(), E = ECs.end(); It != E; ++It) {
     ...
     errs() << "F2I: MinBitwidth=" << MinBW << ", R: " << R << "\n";
     ...
     if (MinBW > MaxRepresentableBits) {
       errs() << "F2I: Value not guaranteed to be representable!\n";
       continue;
     }
     if (MinBW > 64) {
       errs() << "F2I: Value requires more than 64 bits to represent!\n";
       continue;
     }
 ...
 bool Float2IntPass::runImpl(Function &F, const DominatorTree &DT) {
   errs() << "F2I: Looking at function " << F.getName() << "\n";
   // Clear out all state.
 ...
 //static RegisterPass<Float2IntLegacyPass> X("float2int", "Float to int Pass");
