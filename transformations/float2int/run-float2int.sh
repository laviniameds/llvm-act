...
for src in $files; do
    ...
    dir_path="${src_dir}/float2int_results"
    mkdir -p $dir_path
    ...
    # then
        ...
        #for i in $rates; do
            ...
            #echo $opt_perf       
            opt-12 -S -load build/transformations/float2int/libFloat2IntLegacyPass.so -float2int < ${opt} > ${opt_perf}      
            #opt-12 -analyze -dot-cfg ${opt}
            echo "Float2Int done! You can find results in ${dir_path}"        
        #done
    # fi
done 
} 2> float2int.err