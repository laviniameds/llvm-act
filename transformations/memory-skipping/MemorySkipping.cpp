...
namespace {
	//define llvm pass
	struct MemorySkippingPass : public FunctionPass {
		...
		MemorySkippingPass() : FunctionPass(ID) {}
		...
		bool isPerforable(Function &llvm_function, Loop *loop, ScalarEvolution &se){
			...
			if(loop->getInductionVariable(se) != NULL)
				errs() << "there is a induction var" << "\n";
			else
				errs() << "there is NOT a induction var" << "\n";
